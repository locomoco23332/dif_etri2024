        ...
        for ep in range(1,num_epochs+1):
            ...
            for num_of_minibatch,indices in enumerate(sampler):
                ...
                for offset in range(1,prediction_frames):
                    ...
                    curr_pose3 = encode_buffer[:,3,:]
                    curr_pose4 = encode_buffer[:,4,:]
                    curr_pose5 = encode_buffer[:,5,:]
                    curr_pose6 = encode_buffer[:,6,:]
                    curr_pose8 = encode_buffer[:,8,:]
                    curr_pose9 = encode_buffer[:,9,:]
                    curr_pose10 = encode_buffer[:,10,:]
                    curr_pose11 = encode_buffer[:,11,:]
                    curr_pose12 = encode_buffer[:,12,:]
                    curr_vec1=curr_pose1-curr_pose0
                    curr_vec2=curr_pose2-curr_pose0
                    curr_vec3=curr_pose3-curr_pose0
                    curr_vec4 = curr_pose4 - curr_pose0
                    #using vector space in this phase
                    ...
                    curr_pose0_n=vqvae.q_sample(curr_pose0,t,noise)
                    curr_pose5_n=vqvae.q_sample(curr_pose5,t,noise)
                    curr_pose9_n=vqvae.q_sample(curr_pose9,t,noise)
                    ...
                    curr_pose_check=vqvae.q_sample(output,t,n_oise)
                    curr_pose_vec_check_2=curr_pose11-curr_pose2+r*(ground_truth-output)
                    #curr_pose_nn2=vqvae.q_sample(curr_pose2,t)
                    curr_pose_nn2=vqvae.q_sample(curr_pose2,t,noise)
                    #curr_pose_vec_check=vqvae.q_sample(curr_pose_vec_check_2,t,n_oise)
                    curr_pose_vec_check=vqvae.world(curr_pose_vec_check_2,curr_pose2,curr_pose5,curr_pose10,condition,t,noise)
                    #curr_pose_vec2=curr_pose10-output
                    curr_pose_n = vqvae.p_sample(curr_pose_nn2,curr_pose_vec_check,curr_vec2,curr_vec3, t, condition,curr_pose2,curr_pose_vec_check_2) # In this part we using the reinforcement learning that is environment of frame and noise.....
                   # purpose of prediction that using policy in this pesudo RL
                    #curr_pose_nn3=vqvae.q_sample(curr_pose3,t,n_oise)
                    #curr_pose_vec3=curr_pose12-curr_pose3+r2*(curr_pose11-curr_pose_n)

                    #curr_pose_vec_check2=vqvae.world(curr_pose_vec3,curr_pose3,curr_pose6,curr_pose11,condition,t,n_oise)
                   
                    #curr_pose_n2 = vqvae.p_sample(curr_pose_nn3,curr_pose_vec_check2,curr_vec3,curr_vec4, t, condition,curr_pose3,curr_pose_vec3) # In this part we using the reinforcement learning that is environment of frame and noise.....
                    
                    #curr_pose_check3=vqvae.q_sample(curr_pose,t,noise)

                    #curr_pose_vec_check3=vqvae.q_sample(curr_pose_vec_check2,t,n_oise)
                    #curr_pose_n3 = vqvae.p_sample(curr_pose_check3,curr_pose_vec_check3,curr_vec1,curr_vec2, t, condition,curr_pose_n2,ik_ground_vec) # In this part we using the reinforcement learning that is environment of frame and noise.....
                   ...
                   # recon_loss = (curr_pose_n2 - curr_pose11).pow(2).mean(dim=(0,-1))
                   # recon_loss = recon_loss.sum()
                    ...
                    recon_loss3_1 = (curr_pose0_n-ik1).pow(2).mean(dim=(0,-1))
                    recon_loss3_2 =(curr_pose9_n-ik2).pow(2).mean(dim=(0,-1))
                    recon_loss3_3 =(curr_pose5_n-ik_mid).pow(2).mean(dim=(0,-1))
                    ...
                    recon_loss6 = (noise-n_oise).pow(2).mean(dim=(0,-1))
                    recon_loss6 = recon_loss6.sum()
                   # recon_loss_p1 = (curr_pose11-curr_pose_n2).pow(2).mean(dim=(0,-1))
                   # recon_loss_p1 = recon_loss_p1.sum()

                   # recon_loss_p2 = (curr_pose12-curr_pose_n3).pow(2).mean(dim=(0,-1))
                   # recon_loss_p2 = recon_loss_p2.sum()
                    #(recon_loss+loss).backward()
                    (loss+r*recon_loss2+r2*recon_loss3_1+r*recon_loss3_2+recon_loss3_3+recon_loss4+recon_loss5+recon_loss6).backward()
                   ...
                   # ep_latent_loss+= float(recon_loss)/prediction_frames
                    ...
                    ep_noise_loss +=float(recon_loss6)/prediction_frames
          ...
          #  avg_latent_loss =ep_latent_loss/mini_batch
            ...
            print("epoch : {ep}, ep_noise_loss : {ep_recon_loss:0.08f} noise:{n:0.08f},q:{q:0.08f} p:{p:0.08f}  ik_loss:{ik:0.08f},ik_loss2:{ik2:0.08f},ik_loss3:{ik3:0.08f}  vec:{vec:0.08f}   learning_rate : {lr:0.07f} , FPS : {FPS}".format(ep=ep,ep_recon_loss=avg_recon_loss,n=avg_noise_loss,q=avg_kl_loss,p=avg_recon_loss2,ik=avg_ik_recon,ik2=avg_ik_recon2,ik3=avg_ik_recon3,vec=avg_ground_vec_loss,lr=vqvae_optimizer.param_groups[0]['lr'],FPS=int((ep/(end-start))*100)))
            ...
            writer.add_scalar('ep_noise_loss2',avg_noise_loss,ep)
...
def test_dif316(latent,latent2,latent3,latent4,latent5,latent6,latent7,latent8,latent9,condition_l,latent0,vae_output):
    with torch.no_grad():
        ...
        ik_vec=latent_vector9-latent_vector0
        #print(condition.shape)
        #output=settings.DIF1016.q_sample(condition,1)
        #output = settings.DIF1010cur.p_sample(condition,latent_vector)
        #output=settings.DIF1015ori.p_sample(output,latent_vector2,condition)
        #output= settings.DIF1016.p_sample(condition_vec,latent_vector0,latent_vector)
        output = settings.DIF1016.p_sample(latent_vector,curr_vec,condition,curr_vec1,curr_vec2,latent_vector0, latent_vector9,ik_vec)     
